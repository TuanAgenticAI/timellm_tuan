"""
Stock baseline training (MSE only, no dynamic prompts).

This script is intended for fair comparisons on the stock dataset with:
- TimeLLM
- LSTM (baseline)
- PatchTST (baseline)
...
from models import TimeLLM, LSTM, PatchTST
...
def parse_args():
    parser = argparse.ArgumentParser(description="Stock baseline training (MSE only)")

    parser.add_argument("--prediction_type", type=str, default="short_term", choices=["short_term", "mid_term"])

    # Basic config
    parser.add_argument("--task_name", type=str, default="long_term_forecast")
    parser.add_argument("--is_training", type=int, default=1)
    parser.add_argument("--model_id", type=str, default="VCB_stock_baseline")
    parser.add_argument("--model_comment", type=str, default="Baseline-MSE")
    parser.add_argument("--model", type=str, default="TimeLLM", choices=["TimeLLM", "LSTM", "PatchTST"])
    ...
    # Model config (keep TimeLLM args too, so TimeLLM can run without extra modifications)
    ...
    # LLM config (used only by TimeLLM)
...
def build_model(args):
    ...
    if args.model == "LSTM":
        return LSTM.Model(args).float()
    if args.model == "PatchTST":
        return PatchTST.Model(args).float()
    raise ValueError(f"Unknown model: {args.model}")


def train_model(args):
    """Training with standard MSE loss only (no directional component)."""
    ...
    # Load content (static prompt) for TimeLLM; harmless for LSTM/PatchTST.
    ...
    accelerator.print(f"\n{'='*60}")
    accelerator.print("Stock Baseline Training (MSE only)")
    ...
    training_history = {"train_loss": [], "vali_loss": [], "test_loss": [], "vali_mae": [], "test_mae": []}
    ...
    for epoch in range(args.train_epochs):
        ...
        model.train()
        # Use plain python time (local import) to avoid extra top-level deps
        ...
        accelerator.print(f"Epoch {epoch + 1} | Time: {_time.time() - epoch_time:.2f}s | Train Loss: {train_loss_avg:.6f}")
    ...
    if accelerator.is_local_main_process:
        ...
        config_dict["version"] = "stock_baseline_mse"
        config_dict["enhancements"] = "None (baseline MSE only)"
        with open(os.path.join(path, "config.json"), "w") as f:
            json.dump(config_dict, f, indent=2)

    accelerator.wait_for_everyone()
    accelerator.print(f"\n{'='*60}")
    accelerator.print("Stock Baseline MSE Training completed!")
