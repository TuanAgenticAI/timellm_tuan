"""
Stock Price Prediction Training V0 - Baseline + Winrate Logging
Uses original Time-LLM without any enhancements for comparison.
Identical to V0 except: thêm winrate (directional accuracy) log cho train/vali/test.
"""
...
def vali(args, accelerator, model, vali_data, vali_loader, criterion, mae_metric):
    ...
    total_loss = []
    total_mae_loss = []
    total_correct, total_samples = 0, 0  # added vs V0
    ...
    with torch.no_grad():
        for batch in tqdm(vali_loader, desc="Validating"):
            ...
            prev_values = batch_x[:, -1, f_dim].to(accelerator.device)  # added vs V0
    ...
    winrate = total_correct / total_samples * 100 if total_samples > 0 else 0  # added vs V0

    model.train()
    return total_loss, total_mae_loss, winrate  # added winrate vs V0


def parse_args():
    parser = argparse.ArgumentParser(description='Time-LLM Stock Prediction V0 - Baseline + Winrate')
...
def train_model_v0(args):
    """Training with standard MSE loss + winrate logging."""
    ...
    accelerator.print(f"\n{'='*60}")
    accelerator.print(f"Time-LLM Stock Prediction V0 - BASELINE + Winrate")
    ...
    training_history = {
        'train_loss': [], 'train_winrate': [],           # added train_winrate vs V0
        'vali_loss':  [], 'vali_mae':  [], 'vali_winrate':  [],  # added vali_winrate vs V0
        'test_loss':  [], 'test_mae':  [], 'test_winrate':  [],  # added test_winrate vs V0
    }
    ...
    for epoch in range(args.train_epochs):
        iter_count = 0
        train_loss = []
        train_correct, train_total = 0, 0  # added vs V0
        ...
        for i, batch in tqdm(enumerate(train_loader), total=len(train_loader), desc=f"Epoch {epoch+1}"):
            ...
            prev_values  = batch_x[:, -1, f_dim]  # added vs V0
            ...
            correct, total = compute_winrate(
                outputs.detach(), batch_y_target.detach(), prev_values.detach()
            )
        ...
        train_winrate  = train_correct / train_total * 100 if train_total > 0 else 0  # added vs V0

        accelerator.print(
            f"Epoch {epoch+1} | Time: {time.time()-epoch_time:.2f}s | "
            f"Train Loss: {train_loss_avg:.6f} | Train Winrate: {train_winrate:.2f}%"  # added winrate vs V0
        )

        # Validation
        vali_loss, vali_mae, vali_winrate = vali(  # added vali_winrate vs V0
            args, accelerator, model, vali_data, vali_loader, criterion, mae_metric)
        test_loss, test_mae, test_winrate = vali(  # added test_winrate vs V0
            args, accelerator, model, test_data, test_loader, criterion, mae_metric)

        accelerator.print(
            f"         | Vali Loss: {vali_loss:.6f} | Vali MAE: {vali_mae:.6f} | Vali Winrate: {vali_winrate:.2f}% | "
            f"Test Loss: {test_loss:.6f} | Test MAE: {test_mae:.6f} | Test Winrate: {test_winrate:.2f}%"
        )

        training_history['train_loss'].append(float(train_loss_avg))
        training_history['train_winrate'].append(float(train_winrate))   # added vs V0
        training_history['vali_loss'].append(float(vali_loss))
        training_history['vali_mae'].append(float(vali_mae))
        training_history['vali_winrate'].append(float(vali_winrate))     # added vs V0
        training_history['test_loss'].append(float(test_loss))
        training_history['test_mae'].append(float(test_mae))
        training_history['test_winrate'].append(float(test_winrate))     # added vs V0
    ...
    if accelerator.is_local_main_process:
        ...
        config_dict['enhancements'] = 'None (baseline) + Winrate Logging'
