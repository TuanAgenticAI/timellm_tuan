"""
Time-LLM Model with Dynamic Prompts for Stock Prediction
Supports per-sample dynamic prompts (e.g., professor advice based on indicators)
"""
...
class Model(nn.Module):
    """
    Time-LLM Model with Dynamic Prompt Support
    
    This model extends the original Time-LLM to support:
    - Dynamic per-sample prompts (e.g., ChatGPT-generated market analysis)
    - Stock-specific prompt formatting
    - Both static (dataset-level) and dynamic (sample-level) prompts
    """
    ...
    def __init__(self, configs, patch_len=16, stride=8):
        ...
        # Static description (dataset-level)
        if configs.prompt_domain:
            self.description = configs.content
        else:
            self.description = 'Stock price prediction using technical indicators.'
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec, dynamic_prompts=None):
        """
        Forecasting with dynamic prompts
        
        Args:
            dynamic_prompts: List of prompt strings, one per sample. 
                            These are like "professor advice" for each sample.
        """
        ...
        for b in range(x_enc.shape[0]):
            ...
            if dynamic_prompts is not None and batch_idx < len(dynamic_prompts):
                # Use dynamic prompt (ChatGPT-generated professor advice)
                professor_advice = dynamic_prompts[batch_idx]
                prompt_ = (
                    f"<|start_prompt|>"
                    f"Dataset: {self.description} "
                    f"Professor's Analysis: {professor_advice} "
                    f"Task: Forecast the next {self.pred_len} {'day' if self.pred_len == 1 else 'days'} "
                    f"based on the previous {self.seq_len} days. "
